...
function handleSubmit(event) {
    ...
    fetch(`http://localhost:${process.env.PORT}/geoData/${destination}`)
        .then(data => data.json())
        .then((data) => {
            ...
            if (!isForcastChecked) {
                fetch(`http://localhost:${process.env.PORT}/getCurrentWeather/${data.geonames[0].lat}&${data.geonames[0].lng}`)
                ...
                fetch(`http://localhost:${process.env.PORT}/getWeatherForcast/${data.geonames[0].lat}&${data.geonames[0].lng}`)
        ...
        }).then(fetch(`http://localhost:${process.env.PORT}/getPhoto/${destination}`).then(data => data.json()).then(data => {
